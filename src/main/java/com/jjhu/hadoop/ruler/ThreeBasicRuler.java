 ...
 */
public class ThreeBasicRuler extends OneTypeRuler {
    public ThreeBasicRuler(int continueLoginDays, float lowRechargeAmount, float highRechargeAmount,
                ...
                if (Integer.parseInt(items[8]) > caishenInterval)
                    if (Integer.parseInt(items[2]) >= continueLoginDays)
                        if (Integer.parseInt(items[4]) == diandianFlag)
                            result = new ReturnMsg(Integer.parseInt(items[1]), "2");
        return result;
    }
}
